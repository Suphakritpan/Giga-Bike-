            ...
            {products.map(p => (
              <div key={p.id} style={{
                ...
                <div style={{ padding: 12 }}>
                  <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 2, fontFamily: 'var(--font-display)' }}>{p.code}</div>
                  <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', lineHeight: 1.4 }}>
                    {locale === 'th' ? p.nameTh : p.name}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4 }}>{p.material.split(' ')[0]}</div>
                </div>
              </div>
            ))}
          </div>
