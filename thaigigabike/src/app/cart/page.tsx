...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '96px 0' }}>
        ...
        <h2 style={{ fontSize: 26, marginBottom: 8 }}>{t.cart.empty}</h2>
  ...
  const shippingNote = t.cart.calculated

  return (
    <div className="section">
      <div className="container grid-cart">

        {/* Items */}
        <div>
          <h1 style={{ fontSize: 34, marginBottom: 20 }}>{t.cart.title}</h1>
              ...
              return (
                <div key={`${item.product.id}-${item.color}`} style={{ display: 'flex', gap: 16, padding: 16, background: 'var(--bg2)', alignItems: 'center' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 2 }}>{item.product.code}</div>
                    <div style={{ fontSize: 17, fontWeight: 500, color: 'var(--text)', lineHeight: 1.4, marginBottom: 4 }}>{name}</div>
                  ...
                  {/* Price */}
                  <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--green)', fontFamily: 'var(--font-display)', width: 80, textAlign: 'right', flexShrink: 0 }}>
          ...
          <div style={{ marginTop: 12 }}>
            <Link href="/products" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 16, color: 'var(--text2)', textDecoration: 'none' }}>
        ...
        <div className="sticky-panel">
          <div style={{ background: 'var(--bg2)', border: '0.5px solid var(--border)', borderRadius: 12, padding: 20 }}>
            <h3 style={{ fontSize: 19, marginBottom: 16 }}>
              {locale === 'th' ? 'สรุปคำสั่งซื้อ' : 'Order Summary'}
            </h3>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, color: 'var(--text2)', marginBottom: 8 }}>
              <span>{t.cart.subtotal}</span>
              <span>฿{totalPrice.toLocaleString()}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, color: 'var(--text2)', marginBottom: 16 }}>
              <span>{t.cart.shipping}</span>
              <span style={{ color: 'var(--text3)', fontSize: 14 }}>{shippingNote}</span>
            </div>
