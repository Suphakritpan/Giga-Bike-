...
export async function GET(_req: NextRequest, { params }: { params: { ticketId: string } }) {
  ...
  const { data: ticket } = await svc.from('support_tickets').select('*').eq('id', params.ticketId).single()
...
export async function POST(req: NextRequest, { params }: { params: { ticketId: string } }) {
  ...
  const { data: ticket } = await svc.from('support_tickets').select('id, user_id, status').eq('id', params.ticketId).single()
  if (!ticket || ticket.user_id !== user.id) return NextResponse.json({ error: 'Not found' }, { status: 404 })
...
export async function PATCH(req: NextRequest, { params }: { params: { ticketId: string } }) {
  ...
  const { data: ticket } = await svc.from('support_tickets').select('id, user_id').eq('id', params.ticketId).single()
  if (!ticket || ticket.user_id !== user.id) return NextResponse.json({ error: 'Not found' }, { status: 404 })
